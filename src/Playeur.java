...
public class Playeur extends Etre{
    ...
    boolean potionEstAuMax(){
        boolean res = false;
        int i = 0;
        int nbPotion = 0;
        while(objet.size() > i){
            if(objet.get(i).type().equals("potion")){
                nbPotion++;
            }
            i++;
        }
    ...
    int getShield(){
        ...
        for (int i = 0 ; i < objet.size();i++){
            if(objet.get(i).type().equals("protection")){
                res = objet.get(i).force/2;
            }
        }
        return res;
    }
    ...
    void changeDePiece(Chateau c){
        ...
        void changeDePiece(Chateau c,int i){
        for(Pièce p : c.piece){
            if (this.ici.porte.get(i).getdirection().equals(p.getNom())){
                 System.out.println(p.getNom());
                 this.changeDePièce(p);  
